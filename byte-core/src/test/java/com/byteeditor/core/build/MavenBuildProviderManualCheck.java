...
public final class MavenBuildProviderManualCheck {
    ...
    static void mavenCommandForcesBatchModeAndNoColor() {
        MavenBuildProvider provider = new MavenBuildProvider("mvn");
        eq("mavenBuild.package", List.of("mvn", "-B", "-Dstyle.color=never", "-Duser.language=en", "package"), provider.mavenCommand("package"));
        eq("mavenBuild.test", List.of("mvn", "-B", "-Dstyle.color=never", "-Duser.language=en", "test"), provider.mavenCommand("test"));
    }
